...
Every project has a `rag-forge.config.ts` at its root. This is the single source of truth for chunking strategy, retrieval settings, evaluation thresholds, and provider choices. RAG-Forge validates the config at init time — misconfigurations fail fast via Pydantic, before any documents are touched.
